...
const basicAuthMiddleware = async function (request, response, callback) {
    ...
    if (!usePerUserAuth && (username === process.env.username || config.basicAuthUser.username && password === process.env.password || config.basicAuthUser.password)) { //if (!usePerUserAuth && username === config.basicAuthUser.username && password === config.basicAuthUser.password) {
        return callback();
